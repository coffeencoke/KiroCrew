...
- **It does not own the `vendors` container anchor.** This slice lives under
  ...
  was stacked on W01's PR #11066 (now merged), which supplies the anchor. Under
  ...
  present via the stack, `setuptools`' `packages = find:` build discovers
  `kiro_crew.connections.vendors.slack`, so the wheel carries it — verified
  against `main` (W01's #11066, which supplies the anchor, has merged). This
  slice must not work around a missing anchor by creating one itself.
