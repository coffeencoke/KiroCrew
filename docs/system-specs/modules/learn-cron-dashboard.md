...
supplied `model` before persisting it. ACP pins use the shared role-model
validator with the selected model namespace, taking entitlement evidence from
the newest matching provider. Existing callers without a namespace retain their
first-provider selection. An empty or unknown set still fails open because
entitlement is unknowable. The registry
