...
builds the full W01 path (binding → handle → custody gate → live `BindingStore` →
`SecretVault` → production transport → `execute`/`PageWalk`) into the
...
`files.list` walk runs through it over a real vault + store + scripted socket, the
vault credential asserted on the request). The single remaining host step — a
one-line install of that factory into `app["knowledge_connector_runners"]` with
the host-owned verifier/store/vault — is documented in
