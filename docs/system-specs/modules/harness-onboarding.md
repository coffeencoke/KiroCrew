...
| `ACP_BACKENDS_SESSION_EVICTION` | The teardown verb Crew SENDS this harness evicts the session from the adapter's own session map, freeing what it held. Multiplexing is not that claim: a harness can serve N sessions perfectly and still have no verb that disposes one, and the gap shows only on a process that outlives many sessions, where every non-evicting teardown leaves its session addressable with its context resident. `session._bg_runtime_backends` requires this ON TOP of the set above, so a harness that has not declared eviction never reaches the high-churn background path — title generation, suggestions, folders and nav each take their own ephemeral `sessionId`, many per conversation, at a rate the operator never controls. Declare it from the verb Crew sends rather than from what the adapter advertises: codex-acp advertises `session/close` and `session/delete`, Crew's teardown for it is `session/cancel`, and after that cancel the same sessionId still serves a prompt whose `cachedReadTokens` shows the context survived — so codex is out, and its background work falls back to one serialized provider session instead. |
...
| 2 capability sets | Decided for every set: in the transport set `ACP_BACKENDS_ACP_RUNTIME`, out of `ACP_BACKENDS_SESSION_SHARING` because the shared-subagent path cannot resolve a codex continuation, out of `ACP_BACKENDS_SESSION_EVICTION` because the teardown Crew sends it is `session/cancel`, in the session MCP array, the advertised-model capture and the model and effort channels, out of every kiro-family set. All three channel sets were *created* by this work, which is why the tuning channels are three sets rather than one. |
