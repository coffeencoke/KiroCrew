...
This RFC covers two surfaces at different lifecycle stages. The view-state record
is implemented: `useAppViewState` ships in `website/src/app-sdk/viewState.ts` with
AWS Control as its first consumer, and the host-owned cache retention it depends on
ships alongside it (see "Persisted view state is a builtin-only record" below). The
durable-job surface remains proposed: `useAppJob` appears nowhere in `src/kiro_crew`
or `website/src`. The run-lifecycle question that half shares a boundary with is
argued in [`rfc-durable-run-coordinator.md`](rfc-durable-run-coordinator.md).
