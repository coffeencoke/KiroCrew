...
def _url_scan_span(match: re.Match[str]) -> tuple[str, str]:
    ...
    Instead:
      ...
      * AT MOST ONE trailing ``)`` is trimmed, and only when the char
        ...
        keeps that byte (it counts toward the length heuristic). The proven
        closer may be followed by sentence punctuation that the maximal regex
        absorbed; that punctuation is beyond the wrapper and stays beside it;
      * a matching ``'`` or ``"`` is trimmed only when the SAME quote
        immediately precedes the URL AND either the quote is followed by a
        structural field terminator (``,``/``}``/``]``/``:`` or the end of the
        span), or its entire remaining suffix contains only ``)]},.;:!?``.
        This stops compact single-quoted mappings and nested emission wrappers
        at the URL value boundary while preserving ordinary apostrophes inside
        the path/query.

    Neither rule can hide a payload: content before the trimmed wrapper is
    still scanned. No byte in the structural suffix set can carry payload; all
    are delimiters, and no host, base64 run, or hex token can be spelled solely
    with them. The quote and paren branches are disjoint based on the character
    immediately before the URL. The paren rule removes only a single balanced
    enclosing delimiter. Shared by the scan and redact call sites so the two
    spans cannot drift.
    """
    url = match.group(0)
    ...
    # Quote wrapper: retain the original immediate field-boundary rule, and
    # also stop before a suffix made entirely of structural closers and
    # punctuation. Compute the suffix start once so many interior quotes stay
    # linear. The exact ``)]},.;:!?`` set cannot spell a host, base64 run, or
    # hex token, so excluding the whole delimiter-only suffix cannot hide a
    # payload; any URL-content byte keeps an interior quote in the scan.
    if preceding in ("'", '"'):
        ...
        for quote_end, char in enumerate(path_and_query):
            ...
            if char == preceding and (
                not following or following in ",}]:" or quote_end + 1 == structural_suffix_start
            ):
                end = quote_end
                break
    # The paren rule runs after quote trimming, but its ``preceding == "("``
    # proof is disjoint from the quote-wrapper proof, so they cannot conflict.
    # Trailing markdown/enclosing paren: trim AT MOST ONE unbalanced closer —
    # the single ``)`` that closes ``[x](url)`` / ``(url)`` — and ONLY when the
    # char immediately preceding the URL is the matching ``(``. The closer can
    # sit before sentence punctuation absorbed by the maximal URL match; walk
    # only that punctuation to find it. A ``while`` over ``)`` itself would
    # erase a pure payload run, so ``)`` is deliberately absent from this
    # suffix set.
    paren_end = end
    while paren_end > 0 and path_and_query[paren_end - 1] in ".,;:!?":
        paren_end -= 1
    # The wrapper proof closes the residual byte: without the ``preceding``
    # proof, a bare URL whose query legally ends in one unbalanced ``)`` lost
    # that byte and a query of exactly _EXFIL_QUERY_MIN_LEN chars slipped under
    # the length heuristic. With the proof, only the one wrapper byte leaves
    # the URL payload; punctuation after it is already beyond the structurally
    # proven boundary. A balanced ``(bar)`` inside the path is preserved, and
    ...
    if (
        preceding == "("
        and paren_end > 0
        and path_and_query[paren_end - 1] == ")"
        and path_and_query[:paren_end].count(")") > path_and_query[:paren_end].count("(")
    ):
        end = paren_end - 1
