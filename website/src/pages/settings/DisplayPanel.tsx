...
export function DisplayPanel() {
  ...
  const editorMut = useMutation(overlay.mutationOpts<string>({
    queryKey: ['kirocrewConfig'],
  ...
  const hostMut = useMutation({
    ...
    onSuccess: (data: unknown, value: string, token: number) => {
      setHostDraft(null)
      setHostError(null)
      return hostOpts.onSuccess(data, value, token)
    },
  })
  const commitHost = () => {
