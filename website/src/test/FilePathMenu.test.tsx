...
describe('FilePathMenu remote-editor row', () => {
  const REMOTE_PATH = '/local/home/helenars/.kiro/crew/config.json'
  ...
  it('is shown for a directory (opens as a remote workspace)', async () => {
    renderWithProviders(
      <FilePathMenu filePath="/local/home/helenars/.kiro/crew" kind="dir">
  ...
  it('routes the vscode-remote URL through window.location on select', async () => {
    ...
    expect(assigned).toBe(
      'vscode://vscode-remote/ssh-remote+dev-dsk.example.com/local/home/helenars/.kiro/crew/config.json',
    )
  })
})
...
describe('remoteEditorUrl', () => {
  ...
  it('uses each editor id as its scheme', () => {
    ...
    expect(remoteEditorUrl('cursor', 'h', '/x')).toBe('cursor://vscode-remote/ssh-remote+h/x')
    expect(remoteEditorUrl('windsurf', 'h', '/x')).toBe('windsurf://vscode-remote/ssh-remote+h/x')
  })
