...
class TestRemoteEditor:
    ...
    @pytest.mark.asyncio
    @pytest.mark.parametrize("editor", ["", "vscode", "kiro", "cursor", "windsurf"])
    ...
    async def test_valid_host_persists_nested(self, tmp_config) -> None:
        host = "dev-dsk-alias.us-west-2.amazon.com"
        async with TestClient(TestServer(_make_app())) as c:
    ...
    async def test_editor_and_host_nest_together(self, tmp_config) -> None:
        ...
        async with TestClient(TestServer(_make_app())) as c:
            ...
            assert (await _patch(c, "dashboard.remote_editor.host", "host.example.com")).status == 200
        data = json.loads(tmp_config.read_text())
        assert data["dashboard"]["remote_editor"] == {"editor": "vscode", "host": "host.example.com"}
