...
class TestBackendPredicates:
    ...
    def test_codex_is_on_the_runtime_and_its_teardown_still_evicts_nothing(self):
        """codex is a member that is not kiro-shaped, and the fact membership omits.
        ...
        What that membership does not grant is a teardown that disposes a session.
        codex's teardown verb is ``session/cancel``, after which the same sessionId
        still answers and its context is still resident, so codex is absent from
        ACP_BACKENDS_SESSION_EVICTION. Both are pinned here to keep the two
        questions from collapsing into one: a call site that read the transport
        answer where the eviction answer belongs would grow the adapter without
        bound at the rate background sessions are created.
        """
