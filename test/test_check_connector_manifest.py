...
class TestRound26UrlUserinfoRejected:
    """The URL ladder's fifth rung: a component OTHER than the host. The host
    was hardened four times, but userinfo (username/password) was never
    inspected — a ``https://user:secret@example.com`` citation certified clean
    and wrote a credential into the public immutable evidence log."""
...
class TestRound26CrossDriveRelpathSafe:
    """Issue #11276: on Windows CI the repo is on D: while toolcache/temp is on
    C:, so an ``os.path.relpath(x, REPO_ROOT)`` across drives raised
    ``ValueError`` and aborted the whole gate. This is the SAME cross-drive
    class the round-22 fix claimed closed for ``commonpath`` — but the sibling
    ``relpath`` calls were left unguarded. ``_repo_rel`` guards them uniformly."""
